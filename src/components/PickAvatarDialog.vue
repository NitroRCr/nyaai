    ...
    >
      ...
      <div flex="~ col">
        ...
        >
          ...
          <q-item>
            <q-item-section>{{ t('Show Background') }}</q-item-section>
            <q-item-section side>
              <q-toggle
        ...
        >
          ...
          <q-btn
            :label="t('OK')"
            @click="onDialogOK(toRaw(selected))"
            flat
            bg-pri
            text-on-pri
            ml-2
...
import type { AvatarWithTitle } from './AvatarPanel.vue'
import AvatarPanel from './AvatarPanel.vue'
import AAvatar from './AAvatar.vue'
import HueSlider from './HueSlider.vue'
import FileInputArea from './FileInputArea.vue'
import materialSymbols from 'src/utils/material-symbols.json'
...
const presetAvatars = {
  ...
  icons: materialSymbols.map(icon => ({ type: 'icon', icon: `sym_o_${icon}`, title: icon })) as AvatarWithTitle[],
}
...
watch(selected, (to, from) => {
  if (from.type === 'image') {
    mutate(mutators.directDeleteEntities([from.itemId]))
  }
})
